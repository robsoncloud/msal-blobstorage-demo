// Type for the claims table object, where each entry is an array of strings
type ClaimsTable = { [index: number]: string[] };

// Define the type for the input claims object, where each claim is a string or number
interface Claims {
 ...
 * @param {Claims} claims ID token claims
 * @returns ClaimsTable claimsObject
 */
export const createClaimsTable = (claims: Claims): ClaimsTable => {
    let claimsObj: ClaimsTable = {};
    let index = 0;
 ...
 * @param {number} index
 * @param {ClaimsTable} claimsObject
 */
const populateClaim = (
    claim: string,
    value: string | number,
    description: string,
    index: number,
    claimsObject: ClaimsTable
): void => {
    claimsObject[index] = [claim, String(value), description];
};
